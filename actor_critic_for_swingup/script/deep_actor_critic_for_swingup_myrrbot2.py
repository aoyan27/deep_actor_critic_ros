...
class agent:
    ...
    filename_critic_model = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/models/myrrbot2_critic_model.dat"
    filename_actor_model = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/models/myrrbot2_actor_model.dat"

    filename_result = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/results/myrrbot2_result.txt"
    ...
    def reset_client(self, angle):
        ...
        request = LinkState()
        request.link_name = "myrrbot2::myrrbot2_link2"
    ...
    def episode_end_flag(self, s):
        if math.fabs(s[0][0]) > 0.25*math.pi:
            return True
        else:
            return False
    ...
    def main(self, limit_episode, limit_step):
        rospy.init_node('deep_actor_critic_for_swingup_myrrbot2')

        rospy.Subscriber("/joint_effort", Float64, self.effort_callback)

        pub = rospy.Publisher("/myrrbot2/joint1_controller/command", Float64, queue_size = 1)
        ...
        while not rospy.is_shutdown():
            if(self.wait_flag):
                ...
                self.next_state[0][0], self.next_state[0][1] = self.get_joint_properties('myrrbot2_joint1')
                
                self.next_state[0][0] = self.init_theta
                self.next_state[0][1] = self.init_omega
                ...
                if wait_count == self.hz*0.5:
                    self.init_theta = uniform(-0.1, 0.1)
                ...
                if not evaluation_flag:
                    ...
                    if self.episode_end_flag(self.state):
                        print "Episode End!!!!(rolling over 0.25*PI)"
                        #  print "V_list : ", V_list
                        #  max_V = np.max(V_list)
                        #  print "max_V : ", max_V
                    ...
                    if self.episode_end_flag(self.state):
                        print "Evaluation End!!!!!(rolling over 0.25*PI)"
                        max_V = np.max(V_list)
                        print "max_V : ", max_V
                        ave_V = np.average(V_list)
                        print "ave_V : ", ave_V
