...
class agent:
    ...
    filename_critic_model = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/models/myrrbot4_critic_model.dat"
    filename_actor_model = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/models/myrrbot4_actor_model.dat"

    filename_result = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/results/myrrbot4_result.txt"
    ...
    def __init__(self):
        ...
        self.D = [xp.zeros((self.data_size, 1, 2), dtype=xp.float32),
        ...
        self.D_actor = [xp.zeros((self.data_size, 1, 2), dtype=xp.float32),
                        xp.zeros(self.data_size, dtype=xp.float32),
                        xp.zeros(self.data_size, dtype=xp.float32),
                        xp.zeros((self.data_size, 1), dtype=xp.float32),
                        xp.zeros((self.data_size, 1, 2), dtype=xp.float32),
    ...
    def reset_client(self, angle):
        ...
        request = LinkState()
        request.link_name = "myrrbot4::myrrbot4_link2"
        request.pose.position.x = -1.0
        request.pose.position.y = 1.1
    ...
    def experience_replay(self, t):
        if t > self.initial_exploration:
            ...
            n_s_replay = xp.ndarray(shape=(self.replay_size, 1, 2), dtype=xp.float32)
    ...
    def main(self, limit_episode, limit_step):
        rospy.init_node('deep_actor_critic_for_swingup_myrrbot4')

        rospy.Subscriber("/joint_effort", Float64, self.effort_callback)

        pub = rospy.Publisher("/myrrbot4/joint1_controller/command", Float64, queue_size = 1)
        ...
        while not rospy.is_shutdown():
            if(self.wait_flag):
                ...
                state[0][0] = self.init_theta
                state[0][1] = self.init_omega
                next_state[0][0] = self.init_theta
                next_state[0][1] = self.init_omega
                #  print "state : ", state
                #  print "state(main) : [%f %f]" % (math.degrees(state[0][0]), math.degrees(state[0][1]))
                #  print "next_state(main) : ", next_state
                #  print "next_state(main) : [%f %f]" % (math.degrees(next_state[0][0]), math.degrees(next_state[0][1]))
                ...
                if not evaluation_flag:
                    ...
                    #  print "state=%s, next_state=%s" % (state, next_state)
                    next_state[0][0], next_state[0][1] = self.get_joint_properties('myrrbot4_joint1')
                    print "next_state(main) : ", next_state
                    print "next_state(main) : [%f %f]" % (math.degrees(next_state[0][0]), math.degrees(next_state[0][1]))
