...
class agent:
    ...
    filename_critic_model = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/models/myrrbot3_critic_model.dat"
    filename_actor_model = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/models/myrrbot3_actor_model.dat"

    filename_result = "/home/amsl/ros_catkin_ws/src/deep_actor_critic/actor_critic_for_swingup/test_results/results/myrrbot3_result.txt"
    ...
    def reset_client(self, angle):
        ...
        request = LinkState()
        request.link_name = "myrrbot3::myrrbot3_link2"
    ...
    def main(self, limit_episode, limit_step):
        rospy.init_node('deep_actor_critic_for_swingup_myrrbot3')

        rospy.Subscriber("/joint_effort", Float64, self.effort_callback)

        pub = rospy.Publisher("/myrrbot3/joint1_controller/command", Float64, queue_size = 1)
        ...
        while not rospy.is_shutdown():
            if(self.wait_flag):
                wait_count += 1
                if wait_count%20==0:
                    print "Please Wait %d seconds" % (3-wait_count/20)
                ...
                self.next_state[0][0], self.next_state[0][1] = self.get_joint_properties('myrrbot3_joint1')
        ...
        while not rospy.is_shutdown():
            if(self.wait_flag):
                ...
                if not evaluation_flag:
                    ...
                    print "actor_update_times : ", actor_update_times
                    ...
                    if self.episode_end_flag(self.state):
                        ...
                        temp_result = np.array(([[episode_count, max_V, ave_reward]]), dtype=np.float32)
                        ...
                        if episode_count%100==0:
                            ...
                            np.savetxt(self.filename_result, test_result, fmt="%.6f", delimiter=",")
                        ...
                        if count >= 20*20:
                            print "Episode End!!!!(over 20 seconds)"
                            #  print "V_list : ", V_list
                            max_V = np.max(V_list)
                            print "max_V : ", max_V
                            V_list = np.array([])
                    ...
                    time = float(count) / 20.0
                    print "self.state(main) : ", self.state
                    self.action = self.actor(self.state, False)
