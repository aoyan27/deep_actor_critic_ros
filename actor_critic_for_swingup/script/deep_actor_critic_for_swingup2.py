...
class agent:
    ...
    def __init__(self):
        ...
        self.critic_model = chainer.FunctionSet(
                l1 = F.Linear(2, 300),
                l2 = F.Linear(300, 1, initialW=np.zeros((1, 300), dtype=np.float32)),
                )
    ...
    def main(self, limit_episode, limit_step):
        ...
        while not rospy.is_shutdown():
            if(self.wait_flag):
                wait_count += 1
                if wait_count%50==0:
                    print "Please Wait %d seconds" % (3-wait_count/50)
                ...
                position, rate = self.get_joint_properties('joint1')
                print "position, rate : ", position, rate
