...
class agent:
    ...
    def function2(self, theta, omega, torque, t):
        return (-0.01*omega + 9.81*math.sin(theta) - torque)

    def euler(self, init_theta, init_omega, torque):
        dt = 0.0001
        theta = init_theta
        omega = init_omega
        
        numpy_time = np.array([])
        numpy_theta = np.array([])
        numpy_omega = np.array([])
        for i in range(1000000):
            t = dt * i
            
            if i%100 == 0:
                print "time : %f / torque : %f / theta : %f / omega : %f" % (t, torque, theta, omega)
                
                numpy_time = np.append(numpy_time, [t])
                numpy_theta = np.append(numpy_theta, [theta])
                numpy_omega = np.append(numpy_omega, [omega])

            k_theta = self.function1(theta, omega, torque, t)
            k_omega = self.function2(theta, omega, torque, t)

            theta += k_theta * dt
            omega += k_omega * dt
        
        plt.plot(numpy_time, numpy_theta)
        plt.show()
...
if __name__=="__main__":
    ...
    init_omega = 0.0
    torque = 0.0
    deep_actor_critic_agent.euler(init_theta/180.0*math.pi, init_omega, torque)
